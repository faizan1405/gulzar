...
export default function AdminZaichaPage() {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      ...
      const zaichaRes = await fetch(`/api/admin/leads?inquiryType=Zaicha+Inquiry&${q}`, { headers: getHeaders() });
      let result: Lead[] = [];
      if (zaichaRes.ok) {
      ...
      const allRes = await fetch(`/api/admin/leads?${q}`, { headers: getHeaders() });
      if (allRes.ok) {
